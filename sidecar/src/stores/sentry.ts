...
export class SentryStore {
  ...
  triageForConversation(conversationId:string):SentryTriage|null { const row=this.db.query("SELECT * FROM sentry_triages WHERE conversation_id=?").get(conversationId) as Record<string,unknown>|null; return row?hydrateTriage(row):null; }
