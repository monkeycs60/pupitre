...
test('la TODO conserve sa configuration sans envoyer les champs propres aux conversations', () => {
  ...
  const input = buildTodoInput({ provider: 'codex', model: 'gpt-5.6-sol', effort: 'high', speed: 'fast', permissionMode: null, orchestrator: false, subagentPresetId: null, subagentEffort: null, branch: ' TECH-24128 ', ticketKey: 'TECH-24128' }, { message: 'Corriger le formulaire', ticketId: 'ticket-1', autonomy: 'local', integrate: true, checks: 'bun test\n\n bun run build ', attachments: [file] })
...
test('une enquête ne peut pas autoriser intégration et push', () => {
  const input = buildTodoInput({ provider: 'claude', model: 'opus', effort: 'high', speed: 'standard', permissionMode: null, orchestrator: false, subagentPresetId: null, subagentEffort: null }, { message: 'Enquêter', ticketId: null, autonomy: 'investigate', integrate: true, checks: '', attachments: [] })
