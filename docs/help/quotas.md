...
Pupitre lit les limites publiées par Claude Code, Codex CLI et Grok Build. Les
jauges sont des signaux : elles ne changent jamais automatiquement le modèle
choisi.
...
Codex et Grok répondent à une lecture d'état gratuite, que Pupitre fait au
démarrage. Claude, lui, n'expose son quota que **pendant un tour** : Pupitre
lance donc au lancement une sonde minimale sur le modèle le moins cher, et
seulement si le relevé stocké ne couvre plus la fenêtre en cours. Le bouton
**Actualiser** force une relève ; côté Claude, elle consomme ce même tour
minimal.
